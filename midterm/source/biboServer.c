...
int main(int argc, char const *argv[])
{
	...
	if (argc == 3)
	{
		...
		while (is_running && num_clients < maxClients)
		{
			...
			pid_t child_pid = fork();	// Fork a copy of itself to serve the client
			tempClient++;
			num_clients++;
			...
			if (child_pid == -1) {
				...
				// Child process, create client FIFO and open for write-only
				...
				snprintf(client_log_name, MAX_LOG_LEN, "%s/%d.log", dirname, req.pid);
			...
			else {	// Parent process: continue accepting connections
				//num_clients++;
				int status;
				if (waitpid(child_pid, &status, 0) == -1) {	// wait for the child process to finish
					perror("waitpid");
					break;
				}
				num_clients--;	// decrement the number of clients connected
				//tempClient--;
				//client_connected=0;
			}
		}
		close(server_fd);
		unlink(SERVER_FIFO);	// Close server FIFO
	}
	else {
		fprintf(stderr, "Usage: ./biboServer <dirname> <max. #ofClients>\n");
		return -1;
	}
	return 0;
}
